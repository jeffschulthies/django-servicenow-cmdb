...
class CMDBObjectField(models.Model):
    """
    The fields that the object contains in ServiceNow.
    """
    name = models.CharField(max_length=255, unique=False, blank=False)
    type = models.ForeignKey('CMDBObjectType', on_delete=models.CASCADE, null=False, blank=False)
    ...
    def clean(self):
        try:
            if not self.type:
                raise ValidationError("The type can not be empty.")
        except CMDBObjectType.DoesNotExist:
            raise ValidationError("The type can not be empty.")

        # Check is this is a duplicate field for the type.
        ...
        # Model field and model function check
...
class CMDBObject(models.Model):
    ...
    def post(self, access_token):
        ...
        if r.status_code == 403:
            raise ValueError("You do not have permission to access this model")
    ...
    def delete(self, access_token):
        ...
        try:
            ...
            key_val['install_status'] = "Retired"
            r = requests.put(
                url="https://{}.service-now.com/api/now/table/{}/{}".format(settings.SERVICE_NOW_DOMAIN, self.type.endpoint, str(self.service_now_id)),
                headers=service_now_headers,
